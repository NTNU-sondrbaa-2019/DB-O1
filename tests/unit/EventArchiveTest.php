...
class EventArchiveTest extends \Codeception\Test\Unit
{
    ...
    {
        $event = $this->model->getEventById(1);
        $event->title = $testValues['title'];
        $event->date = $testValues['date'];
        $event->description = $testValues['description'];
        $this->model->modifyEvent($event);

        // Id was successfully unmodified
        $this->assertEquals($event->id, 1);

        $this->tester->seeNumRecords(3, 'event');
        // Record was successfully modified
